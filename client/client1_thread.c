...
static void my_publish_callback(struct mosquitto *mosq, void *obj, int mid, int reason_code, const mosquitto_property *properties)
{
    UNUSED(obj);
    UNUSED(reason_code);
    UNUSED(properties);
...
void *client1_body(void *arg)
{
    PCLIENT_ARGS args = (PCLIENT_ARGS)arg;
    int argc = args->argc;
    char **argv = args->argv;
